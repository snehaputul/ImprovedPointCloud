...
random.seed = 42
path = Path("datasets/ModelNet40")  # D:\\Projects\\PhD\\experiments\\project_mosha\\
...
def read_off(file):
    ...
    if line != "OFF":
        line = line[3:]
        n_verts, n_faces, __ = tuple([int(s) for s in line.split(' ')])
    else:
        n_verts, n_faces, __ = tuple([int(s) for s in file.readline().strip().split(' ')])
    verts = [[float(s) for s in file.readline().strip().split(' ') if s is not ''] for i_vert in range(n_verts)]
    faces = [[int(s) for s in file.readline().strip().split(' ') if s is not ''][1:] for i_face in range(n_faces)]
    return verts, faces


with open(path/"airplane/train/airplane_0002.off", 'r') as f:
    ...
    for epoch in range(epochs):
        ...
        if val_loader:
            ...
            if val_acc>best_val_acc:
                ...
                torch.save(pointnet.state_dict(), "modelnet40_best" + ".pth")

            print('Valid accuracy: %.2f %%' % val_acc, flush=True)

        # save the model
        if save:
            torch.save(pointnet.state_dict(), "modelnet40_" + str(epoch) + ".pth")
